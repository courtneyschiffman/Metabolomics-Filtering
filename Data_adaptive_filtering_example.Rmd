...
We now demonstrate how to partition the remaining features along the x-axis based on quantiles (20th,40th,60th,80th) and to visualize and select appropriate filtering cutoffs within each partition (e.g. absolute value of the lower quartile of features below the zero difference line).
...
The function 'filter.icc' identifies features that pass a specified ICC threshold. We subset the data to just those features, giving a final filtered dataset.
